...
const getInitialProviderSettings = (): ProviderSetting => {
  ...
  PROVIDER_LIST.forEach((provider) => {
    initialSettings[provider.name] = {
      ...
      settings: {
        /*
         * Local providers should be disabled by default, except OpenAILike (OpenAI Compatible),
         * which is this app's primary provider and must show up in the homepage provider selector.
         */
        enabled: provider.name === 'OpenAILike' ? true : !LOCAL_PROVIDERS.includes(provider.name),
      },
    };
  });
  ...
  /*
   * Migrate old state: earlier versions persisted OpenAILike as disabled-by-default.
   * If it is still disabled but was never actually configured, enable it so it appears
   * in the homepage provider selector. A user who configured it and explicitly turned it
   * off (baseUrl/apiKey/model present) keeps their choice.
   */
  const openAiLike = initialSettings.OpenAILike;

  if (openAiLike && openAiLike.settings.enabled === false) {
    const { baseUrl, apiKey, model } = openAiLike.settings;

    if (!baseUrl && !apiKey && !model) {
      openAiLike.settings.enabled = true;
    }
  }
