...
const CloudProvidersTab = () => {
  ...
  // Load and filter providers — disable all cloud providers on first load
  // (OpenAILike is the only active provider after simplification)
  useEffect(() => {
    const newFilteredProviders = Object.entries(settings.providers || {})
      .filter(([key]) => !['Ollama', 'LMStudio', 'OpenAILike'].includes(key))
      .map(([key, value]) => {
        const cfg = value as IProviderConfig;
        // Force all cloud providers disabled so they don't appear in ModelSelector
        if (cfg.settings.enabled) {
          settings.updateProviderSettings(key, { ...cfg.settings, enabled: false });
        }
        return {
          name: key,
          settings: { ...cfg.settings, enabled: false },
          staticModels: cfg.staticModels || [],
          getDynamicModels: cfg.getDynamicModels,
          getApiKeyLink: cfg.getApiKeyLink,
          labelForGetApiKey: cfg.labelForGetApiKey,
          icon: cfg.icon,
        };
      });
